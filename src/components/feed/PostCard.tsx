...
import { MapPin, Clock, ArrowRight, Lock, Sparkles, Tag, Flag } from 'lucide-react';
    ...
    return (
      <>
        <Link href={`/ilan/${post.id}`} className="block group">
          <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-xl hover:border-[#312E81]/40 transition-all duration-200 flex flex-col md:flex-row md:items-center justify-between gap-4 relative">
            {/* Main Info */}
      ...
      <Link href={`/ilan/${post.id}`} className="block group h-full">
        <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-md hover:shadow-2xl hover:border-[#312E81]/40 transition-all duration-300 flex flex-col justify-between h-full relative overflow-hidden">
          <div className="space-y-3.5">
            {/* Category & Urgency Bar & Report Button */}
            ...
            <div className="flex items-center justify-between gap-2 pt-0.5 min-h-[40px]">
              ...
              <div className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-600 bg-slate-100/80 px-2 py-0.5 rounded-lg border border-slate-200/60 shrink-0 max-w-[170px] truncate" title={`${post.location_city} / ${post.location_district || ''}`}>
                <MapPin className="w-3 h-3 text-[#312E81] shrink-0" />
                <span className="truncate">
                  {post.location_city}{post.location_district ? ` / ${post.location_district}` : ''}
                </span>
              </div>
            </div>
